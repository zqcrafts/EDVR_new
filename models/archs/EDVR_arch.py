...
'''
@File    :   EDVR.py
@Time    :   2020/08/28 10:59:12
@Author  :   Zeyu Xiao 
@Version :   1.0
@Contact :   zeyuxiao1997@gmail.com   zeyuxiao1997@163.com
@License :   (C)Copyright 2020-2022, USTC, CHN
@Desc    :   EDVR1.0 version，has some difference with current xintao's github page
'''
...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        self.tsa_fusion = TSA_Fusion(nf=self.nf, nframes=self.nframes, center=self.center)

        #### reconstruction
        self.recon_trunk = arch_util.make_layer(ResidualBlock_noBN_f, self.back_RBs)
        #### upsampling
        self.upconv1 = nn.Conv2d(self.nf, self.nf, 3, 1, 1, bias=True)
        self.upconv2 = nn.Conv2d(self.nf, self.nf, 3, 1, 1, bias=True)
        # self.pixel_shuffle = nn.PixelShuffle(2)
        self.HRconv = nn.Conv2d(self.nf, self.nf, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        #### pcd alignTypeError: unsupported operand type(s) for *: 'int' and 'NoneType'
        ...
        base = F.interpolate(x_center, scale_factor=1.0, mode='bilinear', align_corners=False)
        out += base
        return out
...
if __name__ == '__main__':
    ...
    x1 = torch.rand(1, 3, 3, 1080, 720).cuda()
    print(x1.shape)
    parser = argparse.ArgumentParser(description="EDVR")
