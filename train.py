...
import sys
#sys.path.append('/ghome/huangjie/SemiVideo/EDVR')
...
def main():
    ...
    parser.add_argument('--opt', type=str, default='/gdata2/zhuqi/work_dirs/deblur/EDVR/options/train/train_EDVR_M.yml', help='Path to option YAML file.')
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            print('################################')
            print(total_iters)
            # Number of train images: 24,000, iters: 3,000 Total epochs needed: 201 for iters 600,001
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['logger']['print_freq'] == 0:
                ...
                total_loss += logs['l_pix']
                # total_ssim += logs['l_ssim']
                total_psnr += logs['psnr']
                mean_loss = total_loss / print_iter
                # mean_ssim = total_ssim/ print_iter
                mean_psnr = total_psnr / print_iter
                message += '{:s}: {:.4e} '.format('mean_loss', mean_loss)
                # message += '{:s}: {:.4e} '.format('mean_ssim', mean_ssim)
            ...
            #         # pbar.update('Test {}th image'.format(idx))
            #
            #     avg_psnr = avg_psnr / idx
            #
            #     # log
            #     logger.info(
            #         '# Validation # PSNR: {:.4e} Previous best PSNR: {:.4e} Previous best step: {}'.format(
            #             avg_psnr, best_psnr, best_step))
            #
